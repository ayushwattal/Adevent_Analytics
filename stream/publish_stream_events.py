...
#Client and Server Stream topics
client_topic = "client_events"
server_topic = "server_events"


#Pub/Sub topics for Stream data publishing
client_topic_path = publisher.topic_path(project, client_topic)
server_topic_path = publisher.topic_path(project, server_topic)


#Random data generation logic for Ad Impressions and Ad Server events.
...
#Generate 100 Beacons (Ad impressions, Ad server events)
for beacons in range(100):
    ...
    server_ad_event['stimestamp'] = int(time.time()) 
    client_ad_event['ctimestamp'] = server_ad_event['stimestamp']+12
    client_ad_event['clicked'] = random.choice(clicked)
    sleep(1)
    #Sending Server and Client events to pub/sub
    publisher.publish(server_topic_path, bytes(json.dumps(server_ad_event),"utf-8"))
    publisher.publish(client_topic_path, bytes(json.dumps(client_ad_event),"utf-8"))